...
export default function ListItems() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const builtDesc = useTemplate
        ? buildTemplateDescription({
            title: fetched?.title,
            bulletPoints: fetched?.bulletPoints || fetched?.bullet_points || fetched?.feature_bullets || fetched?.features,
            description: fetched?.description,
            images: fetched?.images,
          })
        : buildDetailedDescription(fetched)
              ...
              <div className="card-body space-y-6">
                ...
                <div>
                            ...
                            setReviewDescription(
                              next
                                ? buildTemplateDescription({
                                    ...
                                    bulletPoints: (product as any)?.bulletPoints,
                                    description: (product as any)?.description,
                                    images: (product as any)?.images,
                                  })
                                : buildDetailedDescription(product)
                            )
                          }
                        }}
                        className="w-4 h-4 text-brand-600 rounded border-slate-300"
                  ...
                  <p className="text-xs text-slate-400 mb-2">
                    {useTemplate
                      ? 'A branded listing layout (banner, key features, shipping/returns info) — the source Amazon photo is served through our own link, not a direct Amazon URL.'
                      : 'Plain text description built from the Amazon listing.'}
                  </p>
                  <textarea
                    rows={8}
